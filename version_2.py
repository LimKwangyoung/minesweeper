...
class MineSweeper:
    def __init__(self, level):
        ...
        self.main_frame = tkinter.Frame(self.root, width=ROOT_WIDTH, height=ROOT_HEIGHT)
        self.main_frame.pack()

        for row in range(self.height):
            for col in range(self.width):
                btn = tkinter.Button(self.main_frame, padx=2, pady=2)
                btn.bind('<Button-1>', lambda event, r=row, c=col: self.find_mine(event, r, c))
                btn.bind('<Button-3>', )
